...
The `tr:trace/1` function accepts a [map of options](https://hexdocs.pm/erlang_doctor/0.2.5/tr.html#t:trace_options/0), which include:
...
There are two additional function: `tr:range/1` and `tr:range/2`, which return only one range if it exists. It is possible to pass a [`tr`](https://hexdocs.pm/erlang_doctor/0.2.5/tr.html#t:tr/0) record or an index to `tr:range/1` as well.
